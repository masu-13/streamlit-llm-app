import os
import streamlit as st
from dotenv import load_dotenv
from langchain_openai import ChatOpenAI
from langchain_core.messages import HumanMessage, SystemMessage

# 環境変数の読み込み
load_dotenv()
api_key = os.getenv("OPENAI_API_KEY")

# 専門家の種類とそれぞれのシステムメッセージを定義
EXPERT_ROLES = {
    "ビジネスコンサルタント": "あなたは経験豊富なビジネスコンサルタントです。経営戦略、マーケティング、組織運営などについて専門的なアドバイスを提供します。",
    "プログラミング専門家": "あなたは熟練したソフトウェアエンジニアです。プログラミング、アーキテクチャ設計、ベストプラクティスについて技術的なアドバイスを提供します。",
    "医療アドバイザー": "あなたは医療分野の専門家です。健康管理、予防医学、一般的な医療情報について助言を提供します。（※診断や治療行為ではありません）",
    "教育コーチ": "あなたは教育のプロフェッショナルです。学習方法、教育カリキュラム、生徒の成長支援について専門的なアドバイスを提供します。",
    "マーケティング専門家": "あなたはデジタルマーケティングの専門家です。広告戦略、SNS活用、ブランディング、顧客獲得について実践的なアドバイスを提供します。"
}

def get_llm_response(user_input: str, expert_type: str) -> str:
    """
    LLMからの回答を取得する関数
    
    Args:
        user_input (str): ユーザーからの入力テキスト
        expert_type (str): 選択された専門家の種類
    
    Returns:
        str: LLMからの回答テキスト
    """
    try:
        # ChatOpenAIのインスタンスを作成
        chat = ChatOpenAI(
            model="gpt-3.5-turbo",
            temperature=0.7,
            openai_api_key=api_key
        )
        
        # システムメッセージとユーザーメッセージを作成
        system_message = SystemMessage(content=EXPERT_ROLES[expert_type])
        human_message = HumanMessage(content=user_input)
        
        # LLMに問い合わせ
        response = chat.invoke([system_message, human_message])
        
        return response.content
    
    except Exception as e:
        return f"エラーが発生しました: {str(e)}"

# Streamlitアプリのメイン部分
def main():
    # ページ設定
    st.set_page_config(
        page_title="AI専門家アシスタント",
        page_icon="🤖",
        layout="wide"
    )
    
    # タイトルと説明
    st.title("🤖 AI専門家アシスタント")
    st.markdown("---")
    
    # アプリの概要
    st.markdown("""
    ### 📖 アプリの概要
    このアプリでは、様々な分野の専門家としてAIに相談できます。
    専門家の種類を選択し、質問や相談内容を入力すると、選択した専門家の視点から回答を得ることができます。
    
    ### 🔧 操作方法
    1. **専門家を選択**: ラジオボタンから相談したい分野の専門家を選んでください
    2. **質問を入力**: テキストエリアに質問や相談内容を入力してください
    3. **回答を取得**: 「回答を取得」ボタンをクリックすると、AIが専門家として回答します
    """)
    
    st.markdown("---")
    
    # 2カラムレイアウト
    col1, col2 = st.columns([1, 2])
    
    with col1:
        st.subheader("⚙️ 設定")
        # 専門家の種類を選択するラジオボタン
        expert_type = st.radio(
            "専門家の種類を選択:",
            options=list(EXPERT_ROLES.keys()),
            help="相談したい分野の専門家を選んでください"
        )
        
        st.info(f"**選択中**: {expert_type}")
    
    with col2:
        st.subheader("💬 質問・相談")
        # 入力フォーム
        user_input = st.text_area(
            "質問や相談内容を入力してください:",
            height=150,
            placeholder="ここに質問や相談内容を入力してください..."
        )
        
        # 送信ボタン
        if st.button("🚀 回答を取得", type="primary"):
            if user_input.strip():
                with st.spinner("AIが考えています..."):
                    # LLMから回答を取得
                    response = get_llm_response(user_input, expert_type)
                    
                    # 回答を表示
                    st.markdown("---")
                    st.subheader("✨ 回答")
                    st.markdown(response)
            else:
                st.warning("⚠️ 質問や相談内容を入力してください。")

if __name__ == "__main__":
    main()


